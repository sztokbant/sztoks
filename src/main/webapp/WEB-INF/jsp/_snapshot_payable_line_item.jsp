...
$(document).ready(function() {
  ...
  prepareUpdateForm($("#form_payable_due_date_${account.id}"),
    $("#payable_due_date_${account.id}"),
    $("#new_payable_due_date_${account.id}"),
...
<div class="row" id="account_row_${account.id}">
    ...
    <div class="col col-value editable-liability">
        <form id="form_payable_due_date_${account.id}">
            <span id="payable_due_date_${account.id}">${account.dueDate}</span>
            <span><input id="new_payable_due_date_${account.id}" name="amount" type="text" style="display: none;"/></span>
