...
class CreditCardAccountAvailableCreditUpdateControllerTest extends AccountAjaxControllerTestBase {
    ...
    public void updateCreditCardAvailableCredit_happy() throws Exception {
        ...
        assertThat(jsonNode.get(JSON_AVAILABLE_CREDIT).asText(), is("$2,900.00"));
        assertThat(jsonNode.get(JSON_USED_CREDIT_PERCENTAGE).asText(), is("3.33%"));
        assertThat(jsonNode.get(JSON_REMAINING_BALANCE).asText(), is("$-300.00"));
        assertThat(jsonNode.get(JSON_BALANCE).asText(), is("$100.00"));

        assertThat(jsonNode.get(JSON_ACCOUNT_TYPE).asText(), is(AccountType.LIABILITY.toString()));
        assertThat(jsonNode.get(JSON_TOTAL_FOR_ACCOUNT_TYPE).asText(), is("$100.00"));
        ...
        assertThat(
                ...
                is("$-300.00"));
        assertThat(creditCardTotalsForCurrencyUnit.get(JSON_BALANCE).asText(), is("$100.00"));

        assertThat(jsonNode.get(JSON_NET_WORTH).asText(), is("$-100.00"));

        // Sanity checks (before)
        assertThat(snapshot.getNetWorth(), is(new BigDecimal("-100.00")));
        assertThat(snapshot.getTotalFor(AccountType.ASSET), is(BigDecimal.ZERO));
        assertThat(snapshot.getTotalFor(AccountType.LIABILITY), is(new BigDecimal("100.00")));
